  ...
  obtainAuthorsFromDom: $ => {
    ...
    if (elements) {
      const result = elements.map(element => {
        ...
        const [lastname, firstname] = nameDiv.map(name => name.replace(':', '').trim()); //Find url
        ...
        const isAnonymous = !!lastname.toUpperCase().includes('ANÓNIMO'); //Find extra info

        const [country, birthYear, deathYear] = Q.find('a span.text-smaller').text().replace('-', ':').split(':').map(x => x.trim());
